...
import { useInviteLearners } from '../data/hooks';
import { groupInviteErrors, inviteSchema, parseEmails } from '../utils';
...
const InviteLearnersModal = ({
      ...
      },
      {
        onSuccess: (responseData) => {
          ...
          if (responseData && 'taskId' in responseData) {
            showNotification(
              intl.formatMessage(
                messages['corporate.catalog.invitation.modal.invite.notification.processing'],
              ),
              'info',
            );
            reset({ emails: '', csvFile: null });
        ...
        onError: (error: any) => {
          ...
          if (responseData && (responseData.errors || responseData.createdCount)) {
            ...
            showNotification(
              intl.formatMessage(messages['corporate.catalog.invitation.modal.invite.notification.generic.error']),
              'error',
            );
          }
        },
  ...
  return (
    <ModalLayout
      title={intl.formatMessage(messages['corporate.catalog.invitation.modal.invite.title'])}
      isOpen={isOpen}
      onClose={() => {
        reset({
          emails: '',
      ...
      actions={(
        <Button onClick={handleSubmit(onSubmit)} disabled={!isValid || isSubmitting || isPending}>
          {intl.formatMessage(messages['corporate.catalog.invitation.modal.invite.action'])}
        </Button>
      )}
    >
      <Container className="py-3">
        <Form.Group controlId="inviteLearnersEmail" size="sm">
          <h4 className="text-primary">{intl.formatMessage(messages['corporate.catalog.invitation.modal.invite.manually.title'])}</h4>
          <Form.Label>
            {intl.formatMessage(messages['corporate.catalog.invitation.modal.invite.manually.description'])}
          </Form.Label>
          <Form.Control
            as="textarea"
            rows={6}
            ...
            placeholder={intl.formatMessage(
              messages['corporate.catalog.invitation.modal.invite.manually.input.placeholder'],
            )}
          ...
          {errorDetails && (
            <Form.Control.Feedback type="invalid">
              {intl.formatMessage(messages['corporate.catalog.invitation.modal.invite.notification.error_details_header'])}
        ...
        <Form.Group controlId="inviteLearnersUpload" size="sm" className="mt-4">
          <h4 className="text-primary">{intl.formatMessage(messages['corporate.catalog.invitation.modal.invite.bulk.title'])}</h4>
          <Form.Label>
            {intl.formatMessage(messages['corporate.catalog.invitation.modal.invite.bulk.description'])}
          </Form.Label>
          {csvFile
            ? (
