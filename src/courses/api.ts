...
export const getCourses = async (partnerId:string, catalogId:string, pageIndex, pageSize):Promise<PaginatedResponse<CorporateCourse>> => {
  try {    
    ...
    logError(error);
    return { count: 0, next: null, previous: null, results: [], numPages: 0, currentPage: 1, start: 0 };
  }
};
